...
class WishListItemModel extends ProductModel
{
    ...
    public function __construct($id)
    {
        ...
        if ($id && is_numeric($id)) {
            ...
            if ($this->item) {
                ...
                // Set the initial properties
                $this->item_id     = $this->item->ID;
                $this->product_id  = $this->item->ProductID;
                $this->title       = $this->item->InternalItemID;
                $this->link        = $this->item->AbsoluteLink();
                $this->endpoint    = Controller::join_links(Director::absoluteBaseURL(), 'shop-api/wishlist', $this->item->ID);
                $this->toggle_link = Controller::join_links($this->endpoint, 'toggleVariation');
            }
        }
    ...
    public function moveVariations($add = true, $quantity = 1)
    {
        $this->called_method = 'moveVariation';
        ...
        $this->extend('addOrRemoveVariations');
